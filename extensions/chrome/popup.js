// ========================
// AuraSafe Vault Popup - Enhanced Edition
...
function createLoadingIndicator() {
  ...
  document.querySelector('.entries-container')?.insertBefore(loader, list);
  return loader;
}
...
function renderEntries(entries) {
  ...
  filteredEntries.forEach((entry, index) => {
    ...
    const fillHandler = () => {
      ...
      // Original fill logic preserved exactly
      chrome.tabs.query({ active: true, currentWindow: true }, tabs => {
        if (tabs[0] && tabs[0].id) {
          chrome.tabs.sendMessage(tabs[0].id, { type: 'fill', entry }, (response) => {
    ...
    // Attach click handler to both button and item (original behavior)
...
async function loadEntries(retryCount = 0) {
  ...
  try {
    ...
    // Wait for response via chrome.storage (original logic preserved)
    chrome.storage.local.get(['lastMessage', 'lastUpdated'], (result) => {
      setLoading(false);
  ...
  searchDebounceTimer = setTimeout(() => {
    chrome.storage.local.get(['lastMessage'], (result) => {
      if (result.lastMessage && result.lastMessage.type === 'entries') {
        ...
        const visibleItems = document.querySelectorAll('.vault-item').length;
        if (visibleItems === 0 && e.target.value.length > 0) {
          // No results message already handled in renderEntries
        }
      }
    });
  }, 200);
...
chrome.storage.onChanged.addListener((changes, area) => {
  if (area === 'local' && changes.lastMessage) {
    ...
    } else if (newMsg && newMsg.type === 'connectionStatus') {
      if (newMsg.status === 'connected') {
        updateStatus('Connected to AuraSafe!', 'success');
        loadEntries(); // Refresh entries on reconnect
      }
    }
  }
});

// Initial load with connection status check
loadEntries();
