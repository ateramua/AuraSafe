// AuraSafe Firefox Extension Background Bridge
  ...
  for (let port = PORT_SCAN_START; port <= PORT_SCAN_END; port += 1) {
    ...
    try {
      const response = await abortableFetch(buildBridgeUrl(port, '/bridge/handshake'), {
        ...
        body: JSON.stringify({ clientId: 'aurasafe-extension', browser: 'firefox' }),
...
async function forwardToBridge(payload) {
  try {
    return await requestBridge('/bridge/command', payload);
  } catch (err) {
