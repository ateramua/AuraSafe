// ========================
// AuraSafe Background Service - Firefox Edition
// ========================
  ...
  try {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        // Store in browser.storage for popup access
...
browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  // Handle getLastEntry request (for keyboard shortcut)
  if (request.action === 'getLastEntry') {
    browser.storage.local.get(['lastUsedEntry'], (result) => {
      sendResponse({ entry: result.lastUsedEntry || null });
    });
    return true;
  }
...
// Store last used entry when popup fills credentials
browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === 'fill' && request.entry) {
    browser.storage.local.set({ lastUsedEntry: request.entry });
  }
  return false; // Don't keep channel open for this simple message
});